...
const COUNTERS = [
  {
    ...
    counterDesc: "The number of CloudSQL scaling events that succeeded",
  },
  {
    counterName: COUNTER_NAMES.SCALING_DENIED,
    counterDesc: "The number of CloudSQL scaling events denied",
  },
  {
    counterName: COUNTER_NAMES.SCALING_FAILED,
    counterDesc: "The number of CloudSQL scaling events that failed",
  },
  {
 ...
 * @return {Attributes}
 */
function _getCounterAttributes(cloudsql, method, previousSize, requestedSize) {
  const ret = {
    [ATTRIBUTE_NAMES.CLOUDSQL_PROJECT_ID]: cloudsql.projectId,
    [ATTRIBUTE_NAMES.CLOUDSQL_INSTANCE_ID]: cloudsql.instanceId,
  };
  if (method) {
...
async function incScalingSuccessCounter(
  ...
  CountersBase.incCounter(
    ...
    _getCounterAttributes(cloudsql, method, previousSize, requestedSize),
  );
}
 ...
 * @param {string} reason
 */
async function incScalingDeniedCounter(cloudsql, requestedSize, reason) {
  await pendingInit;
