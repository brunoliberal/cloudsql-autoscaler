...
class ConfigValidator {
  ...
  /**
   * Validates the given object against the CloudSQL Autoscaler Config schema.
   *
   * @param {AutoscalerCloudSQL[]} clusters
   */
...
/**
 * Validates the specified CloudSQL Autoscaler JSON configuration file.
...
/**
 * Validates all the CloudSQL Autoscaler YAML config files specified in the
...
async function assertValidGkeConfigMapFile(configValidator, filename) {
  ...
  for (const configMapFile of Object.keys(configMap.data)) {
    ...
    try {
      const cloudSQLConfig = yaml.load(configMapData);
      await configValidator.parseAndAssertValidConfig(
        JSON.stringify(cloudSQLConfig),
      );
    } catch (e) {
      if (e instanceof ValidationError) {
        console.error(
