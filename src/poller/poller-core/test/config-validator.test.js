    ...
    it("fails when given an empty array", async () => {
      await configValidator
        ...
        .should.be.rejectedWith(
          new ValidationError(
            "Invalid Autoscaler Configuration parameters:\n" +
              "CloudSQLConfig must NOT have fewer than 1 items",
          ),
        );
    });
    it("fails when config does not contain required params", async () => {
      await configValidator
        ...
        .should.be.rejectedWith(
          new ValidationError(
            "Invalid Autoscaler Configuration parameters:\n" +
              "CloudSQLConfig/0 must have required property 'projectId'\n" +
              "CloudSQLConfig/0 must have required property 'instanceId'",
          ),
        );
    });
    it("fails with an invalid property ", async () => {
      await configValidator
        ...
        .should.be.rejectedWith(
          new ValidationError(
            "Invalid Autoscaler Configuration parameters:\n" +
              "CloudSQLConfig/0 must NOT have additional properties",
          ),
        );
    });
